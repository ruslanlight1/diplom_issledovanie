...
def send_links_from_file():
    ...
    now = datetime.now().strftime("%Y-%m-%d_%H-%M")
    out_path = os.path.join(LINK_LOGS_DIR, f"direct_links_{now}.txt")

    with open(out_path, "w", encoding="utf-8") as f:
        for nick in nicknames:
            f.write(f"https://example.com/?user={nick}\n")

    return Response(
        open(out_path, "rb").read(),
        content_type="text/plain",
        headers={"Content-Disposition": f"attachment; filename=direct_links_{now}.txt"}
    )
